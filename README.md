...
* **RFC6851**
    ...
    * Server capability: MOVE
      ...
      If the server supports the MOVE extension, node-imap can move messages to another mailbox atomically. If the server does not, move() falls back to using a workaround (which can very rarely fail, leaving messages in both mailboxes).
